...
echo "Creating and uploading backup to remote:${BUCKET_NAME}/${BACKUP_NAME}.sql.gz"

if ! PGPASSWORD=$POSTGRES_PASSWORD pg_dump -h "$POSTGRES_HOST" -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DATABASE" 2>/dev/stderr | \
    gzip | \
    rclone --s3-no-check-bucket --quiet rcat "remote:${BUCKET_NAME}/${BACKUP_NAME}.sql.gz"; then
